		...
		return (
			<>
				...
				<h1 style={{ opacity: this.state.opacity }}>
					{this.parseNumber(this.props.minutes)}:
					{this.parseNumber(this.props.seconds)}
				</h1>
				<span style={{ opacity: this.state.opacity }}>minutes left</span>
				<button onClick={this.changeTimeVisibility}>
					{this.state.opacity === 1 ? "Hide time" : "Show time"}
				</button>
			</>
		);
	}
